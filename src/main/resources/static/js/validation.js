...
function validateForm(frm){
	...
	$(frm).find('.requiredValFld').each(function(){
		...
		if(! isDisabled(this)){
			...
			var fileAnc = $(this).parent().find('.anchCheck').html();
			console.log("checking for file "+fileAnc);
		...
		/*else if(typ=='file' && typ=='FILE'){
			console.log("checking for general condition");
			if(fileAnc){
				console.log("having file ");
			}
			else{
	...
	if(_validationOutput){
	    $(frm).find('.sizeFld').each(function(){
	    	if(! isDisabled(this)){
	    	if(!isUploadedFileSize(this)){
	    		 _validationOutput=false;
	    		 return  _validationOutput;
	    	}
	    	}
...
function isDecimal(fld) {
	...
	if (obj != '') {
		if (!obj.match(char)) {
			alert("Please enter latitude / longitude value in decimal.");
			fld.focus();
			return false;
		}
	}
...
function isUploadedFileSize(obj){
	var filess = $('.sizeFld:input[type=file]')[0].files;
	console.log("files to process"+filess+" file len "+filess.length);
	var sum;
	if(filess.length > 0){
		for (i = 0; i < filess.length; i++){
			sum = sum + filess[i].size;
			if(sum > 200871520){
				alert("Total size of files should be less than 200 MB");
				return false;
			}
		}
		...
		for (i = 0; i < filess.length; i++)
		{
			...
			console.log("file size "+filess[i].size);
		   if (filess[i].size > 20000000){
			   alert("File "+filess[i].name+" should be less than 20 MB");
				$(obj).focus();
			   return false;
		   }
		else if(filess[i].size < 10){
			   alert("Please select the file that have some data");
			   $(obj).focus();
			   return false;
		   }
		else{
